...
describe("syncYjsMiddleware", () => {
  ...
  beforeEach(() => {
    ...
    const creator = (set: any) => ({
      ...
      addItem: (item: string) =>
        set((state: MyState) => ({ items: [...state.items, item] })),
  ...
  it("should initialize both stores with the same initial state", () => {
    expect(JSON.stringify(store1.getState())).toEqual(
      JSON.stringify(store2.getState())
    );
  });
  ...
  it("should sync changes from store2 to store1", () => {
    ...
    expect(store1.getState().items).toEqual([
      "apple",
      "banana",
      "cherry",
      "elderberry",
